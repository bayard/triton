...
-- Main frame
local listFrame = CreateFrame("Frame", "TritonKeywordListUI", UIParent, "ButtonFrameTemplate");
...
listFrame.TitleText:SetText(addonName);
listFrame.portrait:SetTexture([[Interface\AddOns\Triton\Media\logo]]);
listFrame:Hide();
...
deleteButton:SetNormalTexture([[Interface\AddOns\Triton\Media\trash]]);
deleteButton:SetHighlightTexture([[Interface\AddOns\Triton\Media\trash]]);
...
addButton:SetNormalTexture([[Interface\AddOns\Triton\Media\iplus]]);
addButton:SetHighlightTexture([[Interface\AddOns\Triton\Media\iplus]]);
...
toggleButton:SetNormalTexture([[Interface\AddOns\Triton\Media\on]]);
toggleButton:SetHighlightTexture([[Interface\AddOns\Triton\Media\on]]);
...
local scrollFrame = CreateFrame("ScrollFrame", "TritonKeywordListUIScroll", listFrame.Inset, "UIPanelScrollFrameTemplate2");
...
local function ToggleUpdate()
    if addon.db.global.globalswitch then
        ...
        toggleButton:SetNormalTexture([[Interface\AddOns\Triton\Media\off]]);
        toggleButton:SetHighlightTexture([[Interface\AddOns\Triton\Media\off]]);
        listFrame.portrait:SetTexture([[Interface\AddOns\Triton\Media\logoo]]);
...
local function CreateListElement(pos)
	...
	local item = CreateFrame("Frame", nil, scrollFrame, BackdropTemplateMixin and "BackdropTemplate");
	...
	item.delb:SetHighlightTexture([[Interface\AddOns\Triton\Media\]]);
    ...
    item.disb:SetScript("OnClick", function(self)
        local isOn = addon:ToggleEntry(self:GetParent().searchString:GetText());
        if isOn then
...
function addon:FillList()
	...
	for i, search in ipairs(tkeys) do
        ...
        curdta = addon.db.global.keywords[search];
		curitm.searchString:SetText(search);
		
		if curdta.active then
			curitm.disb:SetNormalTexture([[Interface\AddOns\Triton\Media\on]]);
            curitm.disb:SetHighlightTexture([[Interface\AddOns\Triton\Media\on]]);
            curitm.disb:GetParent():SetBackdropColor(0.2,0.2,0.2,0.8);
            curitm.searchString:SetTextColor(1, 1, 1, 1);
		else
			curitm.disb:SetNormalTexture([[Interface\AddOns\Triton\Media\off]]);
            curitm.disb:SetHighlightTexture([[Interface\AddOns\Triton\Media\off]]);
...
function addon:ShowAddForm(search)
